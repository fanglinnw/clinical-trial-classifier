...
def calculate_binary_metrics(y_true, y_pred, y_prob):
    """Calculate binary classification metrics including AUC."""
    ...
    auc = roc_auc_score(y_true, y_prob[:, 1])  # Use probability of positive class
    ...
    metrics_str = (
        ...
        f"AUC-ROC:   {auc:.3f} (area under ROC curve)\n\n"
        f"Confusion Matrix:\n"
        f"{'=' * 50}\n"
...
def plot_roc_curve(y_true, y_prob_finetuned, y_prob_zeroshot, save_path):
    """Plot ROC curves for both models."""
    plt.figure(figsize=(8, 6))

    # Plot ROC curve for fine-tuned model
    fpr_ft, tpr_ft, _ = roc_curve(y_true, y_prob_finetuned[:, 1])
    plt.plot(fpr_ft, tpr_ft, label=f'Fine-tuned (AUC = {roc_auc_score(y_true, y_prob_finetuned[:, 1]):.3f})')

    # Plot ROC curve for zero-shot model
    fpr_zs, tpr_zs, _ = roc_curve(y_true, y_prob_zeroshot[:, 1])
    plt.plot(fpr_zs, tpr_zs, label=f'Zero-shot (AUC = {roc_auc_score(y_true, y_prob_zeroshot[:, 1]):.3f})')
...
def main():
    ...
    test_texts, test_labels, test_paths = load_dataset(args.test_dir, debug=args.debug,
    ...
    finetuned_metrics, cm_finetuned, auc_finetuned = calculate_binary_metrics(true_labels, predictions, probabilities)
    print(finetuned_metrics)

    print("\nZero-shot Model Results:")
    zeroshot_metrics, cm_zeroshot, auc_zeroshot = calculate_binary_metrics(test_labels, zero_shot_predictions,
    ...
    plot_roc_curve(test_labels, probabilities, zero_shot_probabilities,
